                    ...
                    <div>
                        ...
                        @php
                            $paymentBadgeClass = match ($booking->payment_status) {
                                ...
                                'payment_pending', 'awaiting_payment' => 'bg-amber-50 text-amber-700',
                                default => 'bg-white text-stone-600',
                            };
                        @endphp
                        <div class="mt-2 flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em]">
                            <span class="rounded-full px-3 py-1 {{ $paymentBadgeClass }}">
                                Payment {{ ucwords(str_replace('_', ' ', $booking->payment_status ?: 'unknown')) }}
                            </span>
                            @if ($booking->billplz_bill_id)
                                <span class="rounded-full bg-white px-3 py-1 text-stone-600">Billplz {{ $booking->billplz_bill_id }}</span>
                            @endif
                            @if ($booking->billplz_transaction_status)
                                <span class="rounded-full bg-white px-3 py-1 text-stone-600">Txn {{ strtoupper($booking->billplz_transaction_status) }}</span>
                            @endif
                            @if ($booking->billplz_paid_at)
                                <span class="rounded-full bg-emerald-50 px-3 py-1 text-emerald-700">Paid {{ $booking->billplz_paid_at->format('d M Y H:i') }}</span>
                            @endif
                        </div>
                        @if ($booking->billplz_bill_url)
                            <div class="mt-2 flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.18em]">
                                <a
                                    href="{{ $booking->billplz_bill_url }}"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    class="inline-flex items-center justify-center rounded-full bg-sky-700 px-3 py-1.5 text-white transition hover:bg-sky-800"
                                >
                                    {{ $booking->billplz_paid ? 'Open Billplz Receipt' : 'Open Billplz Bill' }}
                                </a>
                            </div>
                        @endif
